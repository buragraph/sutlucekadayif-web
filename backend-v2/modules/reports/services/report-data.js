...
// Rapor verisi cache'i — aynı dönem için tekrar istendiğinde veritabanına gitmez.
// Eskiden anahtar veri versiyonunu içeriyordu (cross-instance bayatlık kontrolü);
// v2'de versiyon kavramı yok, düz TTL + yazma sonrası invalidate yeterli.
const reportCache = new TtlCache({ stdTTL: 3600, checkperiod: 120 });
 ...
 * @param {number} [_knownVersion] - Kullanılmıyor (eski imza korunuyor: toplu PDF
 *   döngüsü hâlâ bu parametreyle çağırıyor)
 */
export async function buildReportData(subeKod, donemBaslangic, donemBitis, subeData = null, _knownVersion = null) {
  ...
  // ── Overrides ──
  // v2'de `veri_overrides` yok: bütçe alanları kendi kolonlarında, metrikler de
  // öyle. Aşağıdaki `overrides.X ?? donem.Y` kalıbı bilinçli olarak korunuyor —
  // hesap satırları eski dosyayla birebir aynı kalsın diye (boş obje ile hep
  // kolon değeri kullanılır).
  const overrides = {};
